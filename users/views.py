...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if errors:
            ...
            if ldap_auth == 1:
                l =LDAP()
                retno = l.auth_login(username, password)
                if retno:
                    ...
                    if user is None:
                        try:
                            ...
                            user_obj.identity = 'developer'
                            user_obj.save()
                        except:
                            user_obj = UserProfile()
                            user_obj.username = user_obj.objects.get(username=username)
                            user_obj.password = make_password(password)
                            user_obj.save()
...
def test_email(request):
    send_user_email('514796090@qq.com', 'register')
...
def deal_user_add(request):
    ...
    if user_add_form.is_valid():
        ...
        user_obj.email = '{}{}'.format(user_add_form.cleaned_data.get('email'), '@code502.com')
